  ...
  async (job: Job) => {
    ...
    if (job.name === 'send-patient-reminder') {
      ...
      await sendWhatsApp(
        ...
        `Olá ${appointment.patient.name}, lembramos que sua consulta está marcada para daqui 2 dias. Qualquer dúvida, entre em contato conosco.`,
      );
    ...
    if (job.name === 'send-admin-reminder') {
      ...
      await sendWhatsApp(
        masterPhone,
        `📋 Lembrete: O paciente ${appointment.patient.name} possui consulta em 2 dias (${new Date(appointment.appointmentAt).toLocaleDateString('pt-BR')}).`,
      );
